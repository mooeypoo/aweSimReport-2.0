...
/*				$id = $this->uri->segment(3, FALSE, TRUE);*/
			case "generator":
				/* set the variables */
/*				$id = (isset($id) ? $id : $this->uri->segment(4, FALSE, TRUE));*/
...
/*					$action = strtolower($this->input->post('submit', TRUE));
					switch ($action) {
						...
						case 'delete':
							...
							if ($row !== FALSE) {
								if ($row->report_status == 'saved') {
									//exists, and not a published log.
									//go ahead an delete:
...
*/
				...
				if ($id !== FALSE) { /* if there is an ID, it is a previously saved report */
					...
					if ($report !== FALSE) { //it exists!
						...
						$inputVal['txtReportDateStart'] = (($report->report_date_end > 0) ? date('n/j/Y',$report->report_date_end) : '');
					}
				}
